...
@section('admin.content')
  ...
  >
    <div class="products-create-main">
      ...
      {{-- ══ 2B. AI CALL VOICE PREVIEW ══ --}}
      <section
        class="card products-call-voice-card"
        data-product-call-voice
        data-feature-active="{{ $callVoiceFeature['enabled'] ? '1' : '0' }}"
        data-page-name="{{ $callVoiceFeature['page_name'] }}"
        data-default-product-title="{{ $callVoiceFeature['default_product_title'] }}"
        data-default-duration="{{ $callVoiceFeature['default_duration'] }}"
      >
        <div class="card-header">
          <h3 class="card-title">AI Call Voice Preview</h3>
          <div class="products-card-tools">
            <span class="badge badge-warning">{{ $callVoiceFeature['enabled'] ? 'Active' : 'Locked' }}</span>
            <span class="badge badge-info">Auto from Product Name</span>
          </div>
        </div>

        <div class="products-call-voice-layout">
          <div class="products-call-voice-main">
            <div class="products-call-voice-banner">
              <strong>Call confirm / reject voice is a premium feature.</strong>
              <p>Type a product name above to preview how the AI engine will generate the locked script for customers. The script body never changes manually.</p>
            </div>

            <div class="products-call-origin-list">
              <span><strong>{PAGE_NAME}</strong> from <a href="{{ route('admin.order-call') }}">Call Voice Setup</a>: <em data-product-call-voice-page>{{ $callVoiceFeature['page_name'] }}</em></span>
              <span><strong>{PRODUCT_TITLE}</strong> from the Product Name field above</span>
              <span><strong>Language</strong> automatic for all supported languages</span>
            </div>

            <div class="products-call-voice-player">
              <button type="button" class="products-call-voice-play" aria-label="Preview call voice">▶</button>
              <div class="products-call-voice-track">
                <div class="products-call-voice-track-top">
                  <div>
                    <strong data-product-call-voice-title>{{ $callVoiceFeature['default_product_title'] }}</strong>
                    <p data-product-call-voice-status>{{ $callVoiceFeature['enabled'] ? 'Auto voice ready' : 'Demo voice preview ready' }}</p>
                  </div>
                  <span data-product-call-voice-duration>{{ $callVoiceFeature['default_duration'] }}</span>
                </div>

                <div class="products-call-waveform" aria-hidden="true">
                  @foreach ($callVoiceFeature['voice_wave'] as $height)
            ...
            <div class="products-call-script-grid">
              <article class="products-call-script-card">
                <span class="products-call-script-tag">Bangla</span>
                <p data-product-call-script-bn>{{ $callVoiceFeature['default_script_bn'] }}</p>
              </article>

              <article class="products-call-script-card">
                <span class="products-call-script-tag">English</span>
                <p data-product-call-script-en>{{ $callVoiceFeature['default_script_en'] }}</p>
              </article>
            </div>
          </div>

          <aside class="products-call-voice-side">
            <div class="products-call-price-box">
              <span>Package price</span>
              <strong>BDT {{ number_format($callVoiceFeature['price_bdt']) }}/month</strong>
              <p>Enable this package to turn the preview into live AI-generated product call audio.</p>
            </div>

            <div class="products-call-side-note">
              <strong>What happens after activation</strong>
              <ul class="products-call-side-list">
                <li>Product title change triggers a fresh voice script</li>
                <li>Page name is pulled from Call Voice Setup automatically</li>
                <li>Customer language is handled automatically for all supported languages</li>
                <li>Bangla and English previews remain available for review</li>
                <li>Voice preview becomes visible in Products pages</li>
              </ul>
            </div>

            <a href="{{ route('admin.order-call') }}" class="btn btn-primary">Manage Call Voice Feature</a>
          </aside>
        </div>
      </section>
